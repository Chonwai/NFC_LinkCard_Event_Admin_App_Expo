...
## §7 成本、供應與到貨期（11 月活動倒推）

### 7.1 讀寫器成本（2026 年市場估計）

| 方案                              | 單價（USD） |      建議數量       | 總價（USD） | 備註                     |
| --------------------------------- | ----------- | :-----------------: | :---------: | ------------------------ |
| **ACR122U（正品/可驗證）**        | $20–30      |   2（主力＋備援）   |   $40–60    | ⚠️ 停產；需驗證正品來源  |
| **ACR1252U（ACS 在產）**          | $40–55      |          2          |   $80–110   | 推薦替代；驅動支援更好   |
| **Sony RC-S380**（Python 備援）   | $50–70      |      1（備援）      |   $50–70    | 僅當 nfc-pcsc 失敗時啟用 |
| **NTAG215 矽膠手帶 ×500**         | $1.5–3/條   | 500（+ 100 buffer） | $900–1,800  | 活動主題手帶（含包裝）   |
| **NTAG215 PVC 卡 ×500**（若改用） | $0.5–0.8/張 |         500         |  $250–400   | 成本敏感時的替代         |
| **序號貼紙 + 列印**               | ~$0.05/張   |         600         |    ~$30     | 批次追蹤用               |

**讀寫器總成本範圍**：$40–180（依方案）
**標籤總成本範圍**：$280–1,830（依型式與數量）
**合計（11 月活動）**：**$320–2,010 USD**

### 7.2 通路與到貨期（以 2026-09-18 起算）

| 通路                 | ACR122U/1252U 到貨 |   NTAG215 標籤到貨    | 備註                                       |
| -------------------- | :----------------: | :-------------------: | ------------------------------------------ |
| **淘寶/1688**        |       3–7 天       | 5–10 天（量大可更慢） | 最便宜；需驗證正品/品質；**假貨率中高**    |
| **AliExpress**       |      7–15 天       |        7–12 天        | 品質不確定性                               |
| **Amazon（國際版）** |      5–10 天       |           —           | ACR122U 多為第三方賣家                     |
| **澳門/香港本地**    |    即時（現貨）    |        3–5 天         | 最快但最貴（加價 30–100%）；本地電子零件店 |
| **ACS 官方代理商**   |      7–14 天       |           —           | 可保證正品（若買 ACR1252U）                |

### 7.3 ⚠️ 最後可接受下單日（11 月中旬活動倒推）

以 **2026-11-15** 為活動日推算：

| 里程碑                       |                 日期                 | 說明                                    |
| ---------------------------- | :----------------------------------: | --------------------------------------- |
| 活動日                       |                11-15                 | Macau Startup Festival                  |
| 最晚可開始寫卡               |                10-20                 | 需 2 週寫卡 + 驗證                      |
| 寫卡開發完成（工具 + spike） |                10-13                 | 需先有讀寫器做 spike                    |
| **讀寫器 + 標籤必須到手**    |              **09-30**               | 淘寶/本地通路到貨（留 3 天 buffer）     |
| **⚠️ 最後下單日**            | **09-22**（淘寶）/ **09-27**（本地） | 9/22 下單 → 9/25–28 到貨 → 10-20 前寫卡 |

> 📌 **現在是 2026-09-18。最後淘寶下單日是 9 月 22 日（4 天後）。** 若走本地通路，可延到 9 月 27 日。**建議立即行動。**

### 7.4 標籤型式建議（11 月）

| 型式                    | 建議？          | 理由                                 |
| ----------------------- | --------------- | ------------------------------------ |
| **矽膠手帶（NTAG215）** | ✅ **首選**     | 活動主題感強；穿戴方便；可當入場憑證 |
