...
| 🟡 部分可開發 | 1（F-01） | **0.75 / 2.0**（W-29 另計 **0.5**） | W-26 三態 UI 可先做（含 A3 的 +0.25）；W-27 / W-28 等 B-2 / B-3 |
| ❌ 後端阻斷 | 4（F-02 / F-06 / F-07 + F-01 部分） | **6.25** | F-02 **3.75** ＋ F-06 0.5 ＋ F-07 0.5 ＋ F-01 阻斷部分 1.0（W-27/28）＋ W-29 0.5 |
| — | — | **19.25** | **全 W-ID 逐項合計**（30 個 W-ID：批次 1 = 10.0、批次 2 = 6.25、批次 3 = 3.0）；其中 **18.25** = 批次小計合計（排除 🚫 W-04 / W-29）；再扣 **buffer 1.0**（W-13 + W-15）→ **承諾交付 17.25** |
| — | — | **+0.5（用戶側）** | **W-09** = A0 修正，移交前（W0）由用戶執行，**不計入前端** |

> ⚠️ **本表三個 bucket 的列合計（9.5 + 0.75 + 6.25 = 16.5）≠ 總計 19.25**——差額為 **W-07（0.5）、W-13（0.5）、W-14（0.25）、W-32（1.5）、W-33（0.5）** 未歸入任何 bucket（皆非「可立即開發/部分/阻斷」三分法可描述）。此為 **N3 的已知呈現缺口**（pre-existing），**不影響任何 W-ID 的估時或排期**。
...
| **C-18** | **批次 1 時程處置** | (a) **承認批次 1 為 2.5–2.7 週**<br/>(b) 把 buffer pool（W-13 + W-15 = 1.0 日）移出：批次 1 = 8.5 日 ≈ 2.4 週 | **(a)** — 新增者含 1 個 High 修復（A1），不宜延後 | ⚠️ 整體排期 |
