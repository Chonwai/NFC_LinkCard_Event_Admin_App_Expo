...
| **W-11** | 硬編值清理（**26 行中文 / 4 個頁面檔**：`home` 6 ＋ `badges` 11 ＋ `nfc-bind` 8 ＋ `overview` 1；含**新增** `copy.eventStatus.*` / `copy.badgeStatus.*` / `copy.badges.*` 三個命名空間 ＋ `copy.nfc.*` 擴充 ＋ `payloadUrl`） | 你 | 0.25 | P1 | A6/A7 |
...
| **F-01** | **2.0** | W-26（1.0，含 A3 +0.25） / W-27 / W-28 |
...
| v1.0-r5 | 2026-09-16 | ① **§4.1b 的 W-11 範圍更正**：29 行 / 5 檔 → **26 行 / 4 檔**，並更正命名空間措詞（新增 `copy.eventStatus.*` / `copy.badgeStatus.*` / `copy.badges.*`；`copy.nfc.*` 為既有 namespace 之擴充）；② **§3.1 的 F-01 由 `1.75` 更正為 `2.0`**（W-26 已含 A3 的 +0.25）| DELTA 複驗：獨立文件審查 N-1 / OS-1 |
