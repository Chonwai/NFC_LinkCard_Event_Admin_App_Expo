...
> 📌 **產能假設（排期前提，開工前請確認）**：本排期隱含**前端每週可投入約 3.5 人日**（批次 1 = 7.0 人日 ÷ 2 週）。
> 若你的實際產能低於此，批次 1 會溢出到第 3 週並壓縮批次 2 的緩衝。
...
> - **buffer 仍足夠**：① 批次 1（7.0）與後端全部工作**並行**，不佔關鍵路徑；② **B-1b 與 B-2/B-3 可並行**；③ B-6/B-7 為小項（各 0.75）且不在此鏈上；④ **B-8**（Credential，2.0）解鎖批次 3 的 W-31，而 W-32 只依賴批次 1+2 → **亦不在此鏈上**。
