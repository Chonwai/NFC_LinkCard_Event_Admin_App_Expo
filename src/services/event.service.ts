...
export const eventService = {
    ...
    async getMyManagedEvents(params?: {
        ...
        const res = await apiClient.get<ApiResponse<{ events: ManagedEventItem[]; pagination?: Pagination }>>(
            '/api/v1/events/my-managed',
            { params }
        );
        const data = res.data.data;

        const events = (data.events ?? []).map(item => {
            const meta = (item as ManagedEventItem & { _meta?: Partial<ManagedEventItem> })._meta;
            return {
                ...item,
    ...
    async getRegistrations(
        ...
        >(`/api/v1/events/${encodeURIComponent(eventId)}/registrations`, { params });
